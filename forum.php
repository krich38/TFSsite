...
$cooldownw = array(
	$user_yz85_data['cooldown'],
	time(),
	$user_yz85_data['cooldown'] - time()
	);
...
if ($admin && !empty($_POST) || $leader && !empty($_POST)) {
	...
	if ($admin_thread_delete !== false) {
		...
		$access = false;
		if (!$admin) {
			$thread = mysql_select_single("SELECT `forum_id` FROM `yz85_forum_threads` WHERE `id`='$admin_thread_id';");
			$forum = mysql_select_single("SELECT `guild_id` FROM `yz85_forum` WHERE `id`='". $thread['forum_id'] ."';");
			foreach($charData as $char) if ($char['guild'] == $forum['guild_id'] && $char['guild_rank'] == 3) $access = true;
		} else $access = true;

		if ($access) {
			// Delete all associated posts
			mysql_delete("DELETE FROM `yz85_forum_posts` WHERE `thread_id`='$admin_thread_id';");
			// Delete thread itself
			mysql_delete("DELETE FROM `yz85_forum_threads` WHERE `id`='$admin_thread_id' LIMIT 1;");
	...
	if ($admin_thread_close !== false) {
		...
		if ($access) {
			mysql_update("UPDATE `yz85_forum_threads` SET `closed`='1' WHERE `id`='$admin_thread_id' LIMIT 1;");
			//die("UPDATE `yz85_forum_threads` SET `closed`='1' WHERE `id`='$admin_thread_id' LIMIT 1;");
	...
	if ($admin_thread_open !== false) {
		...
		if ($access) {
			mysql_update("UPDATE `yz85_forum_threads` SET `closed`='0' WHERE `id`='$admin_thread_id' LIMIT 1;");
	...
	if ($admin_thread_sticky !== false) {
		...
		if ($access) {
			mysql_update("UPDATE `yz85_forum_threads` SET `sticky`='1' WHERE `id`='$admin_thread_id' LIMIT 1;");
	...
	if ($admin_thread_unstick !== false) {
		...
		if ($access) {
			mysql_update("UPDATE `yz85_forum_threads` SET `sticky`='0' WHERE `id`='$admin_thread_id' LIMIT 1;");
...
if ($admin && !empty($_POST)) {
	...
	if ($admin_board_create_name !== false) {
		...
		// Insert data
		mysql_insert("INSERT INTO `yz85_forum` (`name`, `access`, `closed`, `hidden`, `guild_id`)
	...
	if ($admin_update_category !== false) {
		...
		// Update the category
		mysql_update("UPDATE `yz85_forum` SET
	...
	if ($admin_category_edit !== false) {
		...
		$category = mysql_select_single("SELECT `id`, `name`, `access`, `closed`, `hidden`, `guild_id` 
			FROM `yz85_forum` WHERE `id`='$admin_category_id' LIMIT 1;");
		if ($category !== false) {
			?>
	...
	if ($admin_category_delete !== false) {
		...
		$threads = mysql_select_multi("SELECT `id` FROM `yz85_forum_threads` WHERE `forum_id`='$admin_category_id';");

		// Then loop through all threads, and delete all associated posts:
		foreach($threads as $thread) {
			mysql_delete("DELETE FROM `yz85_forum_posts` WHERE `thread_id`='". $thread['id'] ."';");
		}
		// Then delete all threads
		mysql_delete("DELETE FROM `yz85_forum_threads` WHERE `forum_id`='$admin_category_id';");
		// Then delete the category
		mysql_delete("DELETE FROM `yz85_forum` WHERE `id`='$admin_category_id' LIMIT 1;");
	...
	if ($admin_post_delete !== false) {
		...
		// Delete the post
		mysql_delete("DELETE FROM `yz85_forum_posts` WHERE `id`='$admin_post_id' LIMIT 1;");
...
if (!empty($_GET)) {
	...
	if ($reply_thread !== false && $reply_text !== false && $reply_cid !== false) {
		...
		if ($user_yz85_data['cooldown'] < time()) {
			user_update_yz85_account(array('cooldown'=>(time() + $config['forum']['cooldownPost'])));

			$thread = mysql_select_single("SELECT `closed` FROM `yz85_forum_threads` WHERE `id`='$reply_thread' LIMIT 1;");

			if ($thread['closed'] == 1 && $admin === false) $access = false;
			else $access = true;

			if ($access) {
				mysql_insert("INSERT INTO `yz85_forum_posts` (`thread_id`, `player_id`, `player_name`, `text`, `created`, `updated`) VALUES ('$reply_thread', '$reply_cid', '". $charData[$reply_cid]['name'] ."', '$reply_text', '". time() ."', '". time() ."');");
				if ($config['forum']['newPostsBumpThreads']) mysql_update("UPDATE `yz85_forum_threads` SET `updated`='". time() ."' WHERE `id`='$reply_thread';");
			} else echo '<p><b><font color="red">You don\'t have permission to post on this thread. [Thread: Closed]</font></b></p>';
		} else {
			?>
				<font class="forumCooldown" color="red">Antispam: You need to wait <?php echo ($user_yz85_data['cooldown'] - time()); ?> seconds before you can create or post.</font>
			<?php
		}
	}
	...
	if ($create_thread_cid !== false && $create_thread_title !== false && $create_thread_text !== false && $create_thread_category !== false) {
		if ($user_yz85_data['cooldown'] < time()) {
			user_update_yz85_account(array('cooldown'=>(time() + $config['forum']['cooldownCreate'])));

			$category = mysql_select_single("SELECT `access`, `closed`, `guild_id` FROM `yz85_forum` WHERE `id`='$create_thread_category' LIMIT 1;");
			if ($category !== false) {
				$access = true;
				if (!$admin) {
				...
				if ($access) {
					mysql_insert("INSERT INTO `yz85_forum_threads`
	...
	if ($update_post_id !== false && $update_post_text !== false) {
		// Fetch the post data
		$post = mysql_select_single("SELECT `id`, `player_name`, `text`, `thread_id` FROM `yz85_forum_posts` WHERE `id`='$update_post_id' LIMIT 1;");
		$thread = mysql_select_single("SELECT `closed` FROM `yz85_forum_threads` WHERE `id`='". $post['thread_id'] ."' LIMIT 1;");
		...
		if ($access) {
			mysql_update("UPDATE `yz85_forum_posts` SET `text`='$update_post_text', `updated`='". time() ."' WHERE `id`='$update_post_id';");
	...
	if ($update_thread_id !== false && $update_thread_title !== false && $update_thread_text !== false) {
		...
		$thread = mysql_select_single("SELECT `id`, `player_name`, `title`, `text`, `closed` FROM `yz85_forum_threads` WHERE `id`='$update_thread_id' LIMIT 1;");
		...
		if ($access) {
			mysql_update("UPDATE `yz85_forum_threads` SET `title`='$update_thread_title', `text`='$update_thread_text' WHERE `id`='$update_thread_id';");
	...
	if ($edit_post_id !== false && $edit_post !== false) {
		// Fetch the post data
		$post = mysql_select_single("SELECT `id`, `thread_id`, `text`, `player_name` FROM `yz85_forum_posts` WHERE `id`='$edit_post_id' LIMIT 1;");
		$thread = mysql_select_single("SELECT `closed` FROM `yz85_forum_threads` WHERE `id`='". $post['thread_id'] ."' LIMIT 1;");
		// Verify access
		$access = PlayerHaveAccess($yourChars, $post['player_name']);
		if ($thread['closed'] == 1 && $admin === false) $access = false;
	...
	if ($edit_thread_id !== false && $edit_thread !== false) {
		// Fetch the thread data
		$thread = mysql_select_single("SELECT `id`, `title`, `text`, `player_name`, `closed` FROM `yz85_forum_threads` WHERE `id`='$edit_thread_id' LIMIT 1;");
	...
	if ($getThread !== false) {
		...
		$threadData = mysql_select_single("SELECT `id`, `forum_id`, `player_id`, `player_name`, `title`, `text`, `created`, `updated`, `sticky`, `hidden`, `closed` FROM `yz85_forum_threads` WHERE `id`='$getThread' LIMIT 1;");

		if ($threadData !== false) {

			$category = mysql_select_single("SELECT `hidden`, `access`, `guild_id` FROM `yz85_forum` WHERE `id`='". $threadData['forum_id'] ."' LIMIT 1;");
			...
			if ($access) {
				...
				$posts = mysql_select_multi("SELECT `id`, `player_name`, `text`, `created`, `updated` FROM `yz85_forum_posts` WHERE `thread_id`='". $threadData['id'] ."' ORDER BY `created`;");
				if ($posts !== false) {
...
<p style="font-size: 13px; padding-left: 10px; padding-top: 10px; height: 5px; width: 600px; border-top: 1px solid black;"><b>[b]Bold Text[/b]</b>, [img]<a href="http://www.imgland.net/">Direct Image Link</a>[/img], [center]Cented Text[/center],<br> [link]<a href="http://youtube.com/" target="_BLANK">http://youtube.com/</a>[/link], [color=<font color="green">GREEN</font>]<font color="green">Green Text!</font>[/color], [*] - Dotted [/*]</p><br>
	...
	if ($new_thread_category !== false && $new_thread_cid !== false) {
		...
		$category = mysql_select_single("SELECT `access`, `closed`, `guild_id` FROM `yz85_forum` WHERE `id`='$new_thread_category' LIMIT 1;");
		if ($category !== false) {
			$access = true;
			if (!$admin) {
				if ($category['access'] > $yourAccess) $access = false;
	...
	if ($getCat !== false) {
		...
		$category = mysql_select_single("SELECT `name`, `access`, `closed`, `hidden`, `guild_id` FROM `yz85_forum` WHERE `id`='$getCat' AND `access`<='$yourAccess' LIMIT 1;");
		...
		if ($category !== false) {
			...
			$threads = mysql_select_multi("SELECT `id`, `player_name`, `title`, `sticky`, `closed` FROM `yz85_forum_threads` WHERE `forum_id`='$getCat' ORDER BY `sticky` DESC, `updated` DESC;");

			///// HTML \\\\\
			if ($threads !== false) {
				?>
				<table class="yz85Table table table-bordered table-striped table-hover" id="forumThreadTable">
					<tr class="yellow">
	...
	if (!$admin) $categories = mysql_select_multi(
		"SELECT `id`, `name`, `access`, `closed`, `hidden`, `guild_id` FROM `yz85_forum` WHERE `access`<='$yourAccess' ORDER BY `name`;");
		else $categories = mysql_select_multi("SELECT `id`, `name`, `access`, `closed`, `hidden`, `guild_id` FROM `yz85_forum` ORDER BY `name`;");
	
	$guildboard = false;
	?>
	<table class="yz85Table table table-striped table-hover" id="forumCategoryTable">
		<tr class="yellow">
			<th>Forum Boards</th>
			<?php
			$guild = false;
	...
	if ($guildboard !== false && $guild || $guildboard !== false && $admin) {
		//
		?>
		<table class="table table-striped table-hover yz85Table" id="forumCategoryTable">
			<tr class="yellow">
				<th>Guild Boards</th>
				<?php
				foreach($charData as $char) {
