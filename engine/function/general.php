...
function getCache() {
	$results = mysql_select_single("SELECT `cached` FROM `yz85`;");
	return ($results !== false) ? $results['cached'] : false;
}

function setCache($time) {
	$time = (int)$time;
	mysql_update("UPDATE `yz85` set `cached`='$time'");
}

// Get visitor basic data
function yz85_visitors_get_data() {
	return mysql_select_multi("SELECT `ip`, `value` FROM `yz85_visitors`");
}

// Set visitor basic data
function yz85_visitor_set_data($visitor_data) {
	$exist = false;
	$ip = ip2long(getIP());
	...
	if ($exist && isset($value)) {
		// Update the value
		$value++;
		mysql_update("UPDATE `yz85_visitors` SET `value` = '$value' WHERE `ip` = '$ip'");
	} else {
		// Insert new row
		mysql_insert("INSERT INTO `yz85_visitors` (`ip`, `value`) VALUES ('$ip', '1')");
	}
}

// Get visitor basic data
function yz85_visitors_get_detailed_data($cache_time) {
	$period = (int)time() - (int)$cache_time;
	return mysql_select_multi("SELECT `ip`, `time`, `type`, `account_id` FROM `yz85_visitors_details` WHERE `time` >= '$period' LIMIT 0, 50");
}

function yz85_visitor_insert_detailed_data($type) {
	$type = (int)$type;
	/*
	...
	*/
	...
	if (user_logged_in()) {
		...
		mysql_insert("INSERT INTO `yz85_visitors_details` (`ip`, `time`, `type`, `account_id`) VALUES ('$ip', '$time', '$type', '$acc')");
	} else mysql_insert("INSERT INTO `yz85_visitors_details` (`ip`, `time`, `type`, `account_id`) VALUES ('$ip', '$time', '$type', '0')");
}
...
function array_sanitize(&$item) {
	$item = htmlentities(strip_tags(mysql_yz85_escape_string($item)));
}

function sanitize($data) {
	return htmlentities(strip_tags(mysql_yz85_escape_string($data)));
}
