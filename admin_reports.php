...
// Fetch data from SQL
$reportsData = mysql_select_multi('SELECT id, name, posx, posy, posz, report_description, date, status FROM yz85_player_reports ORDER BY id DESC;');
...
if (!empty($_POST)) {
    ...
    // Update SQL
    mysql_update("UPDATE `yz85_player_reports` SET `status`='$status' WHERE `id`='$reportId' LIMIT 1;");
    ...
    if ($changelogStatus) {
        ...
        $changelog = mysql_select_single("SELECT * FROM `yz85_changelog` WHERE `report_id`='$changelogReportId' LIMIT 1;");
        // If changelog exist
        $updatechangelog = false;
        if ($changelog !== false) {
            // Update it
            mysql_update("UPDATE `yz85_changelog` SET `text`='$changelogText', `time`='$time' WHERE `id`='".$changelog['id']."' LIMIT 1;");
            echo "<h2>Changelog message updated!</h2>";
            $updatechangelog = true;
        } else {
            // Create it
            mysql_insert("INSERT INTO `yz85_changelog` (`text`, `time`, `report_id`, `status`)
        ...
        if ($updatechangelog) {
            ...
            $cache->setContent(mysql_select_multi("SELECT `id`, `text`, `time`, `report_id`, `status` FROM `yz85_changelog` ORDER BY `id` DESC;"));
            $cache->save();
        }
    ...
    if ($price > 0) {
        ...
        if ($account !== false) {
            // transaction log
            mysql_insert("INSERT INTO `yz85_paypal` VALUES ('', '$reportId', 'report@admin.".$user_data['name']." to ".$account['email']."', '".$account['id']."', '0', '".$price."')");
            // Process payment
            $data = mysql_select_single("SELECT `points` AS `old_points` FROM `yz85_accounts` WHERE `account_id`='".$account['id']."';");
            // Give points to user
            $new_points = $data['old_points'] + $price;
            mysql_update("UPDATE `yz85_accounts` SET `points`='$new_points' WHERE `account_id`='".$account['id']."'");
