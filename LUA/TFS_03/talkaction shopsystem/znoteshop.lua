...
function onSay(cid, words, param)
	...
	if getPlayerStorageValue(cid, storage) <= os.time() then
		...
		-- Create the query
		local orderQuery = db.storeQuery("SELECT `id`, `type`, `itemid`, `count` FROM `yz85_shop_orders` WHERE `account_id` = " .. accid .. " LIMIT 1;")
					...
					if playerCap >= itemweight and getTileInfo(getCreaturePosition(cid)).protection then
						...
						if(backpack and backpack.itemid > 0) then
							...
							if(received ~= false) then
								db.executeQuery("DELETE FROM `yz85_shop_orders` WHERE `id` = " .. q_id .. ";")
