...
if($undelete_id) {
	...
	$undelete_q1 = mysql_select_single('SELECT `character_name` FROM `yz85_deleted_characters` WHERE `done` = 0 AND `id` = ' . $undelete_id . ' AND `original_account_id` = ' . $session_user_id . ' AND NOW() < `time`');
	if($undelete_q1) {
		mysql_delete('DELETE FROM `yz85_deleted_characters` WHERE `id` = ' . $undelete_id);
	...
	if (!empty($_POST['action'])) {
		...
		switch($action) {
			...
			case 'change_name':
				...
				$order = mysql_select_single("SELECT `id`, `account_id` FROM `yz85_shop_orders` WHERE `type`='4' LIMIT 1;");
				if ($order !== false) {
						...
						if (!$exist) {
							...
							if ($newname !== false) {
								...
								if ($error === false) {
									...
									mysql_delete("DELETE FROM `yz85_shop_orders` WHERE `id`='".$order['id']."' LIMIT 1;");
								}
			...
			case 'change_comment':
				...
				if (user_character_account_id($char_name) === $session_user_id) {
					$comment_data = user_yz85_character_data(user_character_id($char_name), 'comment');
					?>
	...
	if ($pending_delete) {
		foreach($pending_delete as $delete) {
			if(new DateTime($delete['time']) > new DateTime())
				...
				mysql_update('UPDATE `yz85_deleted_characters` SET `done` = 1');
