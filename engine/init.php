...
if (user_logged_in() === true) {
	...
	$user_yz85_data = user_yz85_account_data($session_user_id, 'ip', 'created', 'points', 'cooldown');
}
$errors = array();
// Log IP
if ($config['log_ip']) {
	...
	if ($flush != false) {
		...
		if (getCache() < $timef) {
			...
			mysql_delete("DELETE FROM yz85_visitors_details WHERE time <= '$timef'");
			setCache($time);
		}
	}
	
	$visitor_data = yz85_visitors_get_data();
	
	yz85_visitor_set_data($visitor_data); // update or insert data
	yz85_visitor_insert_detailed_data(0); // detailed data
	
	$visitor_detailed = yz85_visitors_get_detailed_data($visitor_config['time_period']);
...
if ($config['allowSubPages']) {
	...
	if (isset($subpages) && !empty($subpages)) {
		...
		<div style="background-color: white; padding: 20px; width: 100%; float:left;">
			...
			<p style="color: black;">The layout is running an outdated sub system which is not compatible with this version of yz85 AAC.</p>
			<p style="color: black;">The file /layout/sub.php is outdated.
			<br>Please update it to look like <a style="color: orange;" target="_BLANK" href="https://github.com/yz85/yz85AAC/blob/master/layout/sub.php">THIS.</a>
			</p>
		</div>
		<?php
	}
