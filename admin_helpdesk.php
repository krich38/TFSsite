...
if ($view !== false){
	if (!empty($_POST['reply_text'])) {
		...
		mysql_update("UPDATE `yz85_tickets` SET `status`='Staff-Reply' WHERE `id`='$view' LIMIT 1;");
	}
	...
	<?php
	$tickets = mysql_select_multi("SELECT id,subject,creation,status FROM yz85_tickets ORDER BY creation DESC");
	if ($tickets !== false) {
		?>
		<table>
			<tr class="yellow">
