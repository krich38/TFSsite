...
if (empty($_POST) === false) {
	if ($config['log_ip']) {
		yz85_visitor_insert_detailed_data(5);
	}
	...
	if (empty($username) || empty($password)) {
		$errors[] = 'You need to enter a username and password.';
			...
			$errors[] = 'That is the public account.';
		...
		if ($login === false) {
			...
			if ($config['mailserver']['register']) {
				$authenticate = mysql_select_single("SELECT `id` FROM `yz85_accounts` WHERE `account_id`='$login' AND `active`='1' LIMIT 1;");
			...
			if ($status) {
				...
				// if IP is not set (etc acc created before yz85 AAC was in use)
				$yz85_data = user_yz85_account_data($login);
				if ($yz85_data['ip'] == 0) {
					$update_data = array(
					'ip' => ip2long(getIP()),
					);
					user_update_yz85_account($update_data);
				}
