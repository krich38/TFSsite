-- yz85 Shop v1.0 for yz85 AAC on TFS 0.3.6+ Crying Damson.
function onSay(cid, words, param)
	...
	if getPlayerStorageValue(cid, storage) <= os.time() then
		...
		-- Create the query
		local orderQuery = db.storeQuery("SELECT `id`, `type`, `itemid`, `count` FROM `yz85_shop_orders` WHERE `account_id` = " .. accid .. ";")
			...
			if q_type == 1 then
				...
				local itemweight = getItemWeightById(q_itemid, q_count)
					if playerCap >= itemweight then
						local delete = db.storeQuery("DELETE FROM `yz85_shop_orders` WHERE `id` = " .. q_id .. ";")
						result.free(delete)
