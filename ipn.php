<?php
	...
	mysql_insert("INSERT INTO `yz85_paypal` VALUES ('', '$txn_id', 'Connection from IP: $connectedIp', '0', '0', '0')");
	...
	if ($status) {
		...
		if ($payment_status == 'Completed') {
			...
			$txn_id_check = mysql_select_single("SELECT `txn_id` FROM `yz85_paypal` WHERE `txn_id`='$txn_id'");
			if ($txn_id_check !== false) {
				...
				if ($receiver_email == $paypal['email']) {
					...
					if ($status) {
						// transaction log
						mysql_insert("INSERT INTO `yz85_paypal` VALUES ('', '$txn_id', '$payer_email', '$custom', '".$paidMoney."', '".$paidPoints."')");
						
						// Process payment
						$data = mysql_select_single("SELECT `points` AS `old_points` FROM `yz85_accounts` WHERE `account_id`='$custom';");

						// Give points to user
						$new_points = $data['old_points'] + $paidPoints;
						mysql_update("UPDATE `yz85_accounts` SET `points`='$new_points' WHERE `account_id`='$custom'");
					}
				}  else {
					$pmail = $paypal['email'];
					mysql_insert("INSERT INTO `yz85_paypal` VALUES ('', '$txn_id', 'ERROR: Wrong mail. Received: $receiver_email, configured: $pmail', '0', '0', '0')");
				}
			}
		}
	} else {
		// Something is wrong
		mysql_insert("INSERT INTO `yz85_paypal` VALUES ('', '$txn_id', 'ERROR: Invalid data. $postdata', '0', '0', '0')");
	}
?>