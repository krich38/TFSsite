...
/* Below HTML CODE = Plugin you can put anywhere in yz85 AAC where you want search.
<h1>Search forum</h1>
...
if ($type !== false && $text !== false && $type <= 4 || $type > 4 && $type <= 6) {
	$forums = mysql_select_multi("SELECT `id` FROM `yz85_forum` WHERE `access`='1' AND `guild_id`='0';");
	$allowedForums = array();
	...
	switch ($type) {
		case 1: // Search titles
			$results = mysql_select_multi("SELECT `id` AS `thread_id`, `forum_id`, `title`, `text`, `player_name` FROM `yz85_forum_threads` WHERE $textTitleSql ORDER BY `id` DESC LIMIT $searchResults;");
		...
		case 2: // Search posts
			$results = mysql_select_multi("SELECT `thread_id`, `player_name`, `text` FROM `yz85_forum_posts` WHERE $textPostSql ORDER BY `id` DESC LIMIT $searchResults;");
			// Missing ['forum_id'], ['title'], lets get them
			for ($i = 0; $i < count($results); $i++) {
				// $results[$i]['asd']
				$thread = mysql_select_single("SELECT `forum_id`, `title` FROM `yz85_forum_threads` WHERE `id`='".$results[$i]['thread_id']."' LIMIT 1;");
				if ($thread !== false) {
		...
		case 3: // Search authors last threads
			$results = mysql_select_multi("SELECT `id` AS `thread_id`, `forum_id`, `title`, `text`, `player_name` FROM `yz85_forum_threads` WHERE $textAuthorSql ORDER BY `id` DESC LIMIT $searchResults;");
			// Filter out search results in custom access boards.
			for ($i = 0; $i < count($results); $i++) 
				if (!in_array($results[$i]['forum_id'], $allowedForums)) 
					$results[$i]['forum_id'] = false;
		...
		case 4: // Search authors last posts
			$results = mysql_select_multi("SELECT `thread_id`, `player_name`, `text` FROM `yz85_forum_posts` WHERE $textAuthorSql ORDER BY `id` DESC LIMIT $searchResults;");
			// Missing ['forum_id'], ['title'], lets get them
			for ($i = 0; $i < count($results); $i++) {
				// $results[$i]['asd']
				$thread = mysql_select_single("SELECT `forum_id`, `title` FROM `yz85_forum_threads` WHERE `id`='".$results[$i]['thread_id']."' LIMIT 1;");
		...
		case 5: // Search latest titles
			$results = mysql_select_multi("SELECT `id` AS `thread_id`, `forum_id`, `title`, `text`, `player_name` FROM `yz85_forum_threads` ORDER BY `id` DESC LIMIT $searchResults;");
			// Filter out search results in custom access boards.
			for ($i = 0; $i < count($results); $i++) 
				if (!in_array($results[$i]['forum_id'], $allowedForums)) 
					$results[$i]['forum_id'] = false;
		...
		case 6: // Search posts
			$results = mysql_select_multi("SELECT `thread_id`, `player_name`, `text` FROM `yz85_forum_posts` ORDER BY `id` DESC LIMIT $searchResults;");
			// Missing ['forum_id'], ['title'], lets get them
			for ($i = 0; $i < count($results); $i++) {
				// $results[$i]['asd']
				$thread = mysql_select_single("SELECT `forum_id`, `title` FROM `yz85_forum_threads` WHERE `id`='".$results[$i]['thread_id']."' LIMIT 1;");
