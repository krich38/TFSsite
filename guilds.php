...
if ($highest_access >= 2) {
	...
	if (!empty($_POST['forumGuildId'])) {
		if ($config['forum']['guildboard'] === true) {
			$forumExist = mysql_select_single("SELECT `id` FROM `yz85_forum` WHERE `guild_id`='$gid' LIMIT 1;");
				if ($forumExist === false) {
					// Insert data
		...
		if (!empty($_POST['warinvite'])) {
			if (get_guild_id($_POST['warinvite'])) {
				...
				if ($status == false && $wars == false) {
					...
					mysql_insert("INSERT INTO `yz85_guild_wars` (`limit`) VALUES ('$limit');");
					header('Location: guilds.php?name='. $_GET['name']);
					exit();
	...
	?>
		...
		<?php
		$i = 0;
		$wars = mysql_select_multi("SELECT `guild1`, `guild2`, `name1`, `name2`, `started`, (SELECT `limit` FROM `yz85_guild_wars` WHERE `yz85_guild_wars`.`id` = `guild_wars`.`id`) AS `limit` FROM `guild_wars` WHERE (`guild1` = '$gid' OR `guild2` = '$gid') AND `status` = 0 ORDER BY `started` DESC");
