-- yz85 Shop v1.0 for yz85 AAC on TFS 1.0+.
function onSay(cid, words, param)
	...
	if player:getStorageValue(storage) <= os.time() then
		...
		-- Create the query
		local orderQuery = db.storeQuery("SELECT `id`, `type`, `itemid`, `count` FROM `yz85_shop_orders` WHERE `account_id` = " .. player:getAccountId() .. " LIMIT 1;")
