...
function fetchImages($status) {
	$status = (int)$status;
	return mysql_select_multi("SELECT `id`, `title`, `desc`, `date`, `status`, `image`, `account_id` FROM yz85_images WHERE `status`='$status' ORDER BY `date` DESC;");
}
...
function insertImage($account_id, $title, $desc, $image) {
	...
	$time = time();
	mysql_insert("INSERT INTO `yz85_images` (`title`, `desc`, `date`, `status`, `image`, `account_id`) VALUES ('$title', '$desc', '$time', '1', '$image', '$account_id');");
}

function updateImage($id, $status) {
	$id = (int)$id;
	$status = (int)$status;
	mysql_update("UPDATE `yz85_images` SET `status`='$status' WHERE `id`='$id';");
}
...
function fetchAllNews() {
	return mysql_select_multi("SELECT `n`.`id`, `n`.`title`, `n`.`text`, `n`.`date`, `p`.`name` FROM `yz85_news` AS `n` INNER JOIN `players` AS `p` ON `n`.`pid` = `p`.`id` ORDER BY `n`.`id` DESC;");
}
...
function shop_delete_row_order($rowid) {
	$rowid = (int)$rowid;
	mysql_delete("DELETE FROM `yz85_shop_orders` WHERE `id`='$rowid';");
}

function shop_update_row_count($rowid, $count) {
	$rowid = (int)$rowid;
	$count = (int)$count;
	mysql_update("UPDATE `yz85_shop_orders` SET `count`='$count' WHERE `id`='$rowid'");
}

function shop_account_gender_tickets($accid) {
	$accid = (int)$accid;
	return mysql_select_multi("SELECT `id`, `count` FROM `yz85_shop_orders` WHERE `account_id`='$accid' AND `type`='3';");
}
...
function user_update_comment($char_id, $comment) {
	...
	$comment = sanitize($comment);
	mysql_update("UPDATE `yz85_players` SET `comment`='$comment' WHERE `player_id`='$char_id'");
}
...
function user_delete_character($char_id) {
	...
	mysql_delete("DELETE FROM `yz85_players` WHERE `player_id`='$char_id';");
}
...
function user_delete_character_soft($char_id) {
	...
	if(!user_character_pending_delete($char_name))
		mysql_insert('INSERT INTO `yz85_deleted_characters`(`original_account_id`, `character_name`, `time`, `done`) VALUES(' . $original_acc_id . ', "' . $char_name . '", (NOW() + INTERVAL ' . config('delete_character_interval') . '), 0)');
	else
		return false;
}
...
function user_character_pending_delete($char_name) {
	...
	$result = mysql_select_single('SELECT `done` FROM `yz85_deleted_characters` WHERE `character_name` = "' . $char_name . '"');
...
function user_pending_deletes($acc_id) {
	$acc_id = (int)$acc_id;
	return mysql_select_multi('SELECT `id`, `character_name`, `time` FROM `yz85_deleted_characters` WHERE `original_account_id` = ' . $acc_id . ' AND `done` = 0');
}
...
*/
	...
	if (user_account_fields_verify_value($verify_data)) {
		...
		if ($player_account_id == $account_id) {
			...
			// Fetch IP data
			$ip_data = user_yz85_account_data($account_id, 'ip');
			if ($ip == $ip_data['ip']) {
...
// Update yz85_accounts table, make sure user is logged in for this. This is used to etc update lastIP
function user_update_yz85_account($update_data) {
	$update = array();
	array_walk($update_data, 'array_sanitize');
	...
	mysql_update("UPDATE `yz85_accounts` SET ". implode(', ', $update) ." WHERE `account_id`=". $user_id .";");
}
...
function user_character_set_hide($char_id, $value) {
	...
	mysql_update("UPDATE `yz85_players` SET `hide_char`='$value' WHERE `player_id`=$char_id");
}
...
function user_create_account($register_data, $maildata) {
	...
	mysql_insert("INSERT INTO `yz85_accounts` (`account_id`, `ip`, `created`, `activekey`) VALUES ('$account_id', '$ip', '$created', '$activeKey')");
...
function user_create_character($character_data) {
	...
	// This is TFS 0.2 compatible import data with yz85 AAC mysql schema
	$import_data = array(
	...
	echo 2;
	mysql_insert("INSERT INTO `yz85_players`(`player_id`, `created`, `hide_char`, `comment`) VALUES ('$charid', '$created', '0', '');");
}
...
// return query data from yz85_players table
function user_yz85_character_data($character_id) {
	$data = array();
	...
	if ($func_num_args > 1)  {
		...
		$fields = '`'. implode('`, `', $func_get_args) .'`';
		$data = mysql_select_single("SELECT $fields FROM `yz85_players` WHERE `player_id` = $charid;");
		return $data;
	}
}

// return query data from yz85 table
// usage: $yz85AAC = user_yz85_data('version');
// echo $yz85AAC['version'];
function user_yz85_data() {
	$data = array();
	...
	if ($func_num_args > 0)  {
		...
		$fields = '`'. implode('`, `', $func_get_args) .'`';
		return mysql_select_single("SELECT $fields FROM `yz85`;");
	} else return false;
}

// return query data from yz85_accounts table
// See documentation on user_data. This fetches information from yz85_accounts table.
function user_yz85_account_data($account_id) {
	$data = array();
	...
	if ($func_num_args > 1)  {
		...
		$fields = '`'. implode('`, `', $func_get_args) .'`';
		return mysql_select_single("SELECT $fields FROM `yz85_accounts` WHERE `account_id` = $accid LIMIT 1;");
	} else return false;
}

// return query data from yz85_visitors table
// See documentation on user_data, but this uses $longip instead. 
function user_yz85_visitor_data($longip) {
	$data = array();
	$longip = (int)$longip;
	...
	if ($func_num_args > 1)  {
		...
		$fields = '`'. implode('`, `', $func_get_args) .'`';
		return mysql_select_single("SELECT $fields FROM `yz85_visitors` WHERE `ip` = $longip;");
	} else return false;
}

// return query data from yz85_visitors_details table
// See documentation on user_data, but this uses $longip instead. 
function user_yz85_visitor_details_data($longip) {
	$data = array();
	$longip = (int)$longip;
	...
	if ($func_num_args > 1)  {
		...
		$fields = '`'. implode('`, `', $func_get_args) .'`';
		return mysql_select_single("SELECT $fields FROM `yz85_visitors_details` WHERE `ip` = $longip;");
	} else return false;
}
...
function user_character_hide($username) {
	...
	$char = mysql_select_single("SELECT `hide_char` FROM `yz85_players` WHERE `player_id`='$username';");
