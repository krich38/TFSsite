...
if (user_logged_in()) {
	if (is_admin($user_data)) {
		...
		if (isset($_POST['delete'])) {
			...
			if ($delete && $action == 1) {
				mysql_delete("DELETE FROM `yz85_changelog` WHERE `id`='$delete' LIMIT 1;");
			...
			if ($status) {
				// POST update
				if ($changelogId > 0) {
					mysql_update("UPDATE `yz85_changelog` SET `text`='$changelogText' WHERE `id`='$changelogId' LIMIT 1;");
					echo "<h2>Changelog message updated!</h2>";
					$updateCache = true;
				} else {
					// POST create
					$time = time();
					mysql_insert("INSERT INTO `yz85_changelog` (`text`, `time`, `report_id`, `status`) VALUES ('$changelogText', '$time', '0', '35');");
		...
		if ($action === 2) {
			$old = mysql_select_single("SELECT `text` FROM `yz85_changelog` WHERE `id`='$changelogId' LIMIT 1;");
		}
...
if ($updateCache === true) {
	$changelogs = mysql_select_multi("SELECT `id`, `text`, `time`, `report_id`, `status` FROM `yz85_changelog` ORDER BY `id` DESC;");
