...
if (empty($_POST) === false) {
	...
	if ($action === 'd') {
		...
		mysql_delete("DELETE FROM `yz85_news` WHERE `id`='$id';");
	...
	if ($action === 'i') {
		...
		list($charid, $title, $text) = array((int)$_POST['selected_char'], mysql_yz85_escape_string($_POST['title']), mysql_yz85_escape_string($_POST['text']));
		$date = time();
		mysql_insert("INSERT INTO `yz85_news` (`title`, `text`, `date`, `pid`) VALUES ('$title', '$text', '$date', '$charid');");
		// Reload the cache.
		$cache = new Cache('engine/cache/news');
		$news = fetchAllNews();
		$cache->setContent($news);
	...
	if ($action === 's') {
		...
		list($title, $text) = array(mysql_yz85_escape_string($_POST['title']), mysql_yz85_escape_string($_POST['text']));
		mysql_update("UPDATE `yz85_news` SET `title`='$title',`text`='$text' WHERE `id`='$id';");
		$cache = new Cache('engine/cache/news');
		$news = fetchAllNews();
		$cache->setContent($news);
		$cache->save();
