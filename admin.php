...
if (empty($_POST) === false) {
	...
	if (empty($_POST['points_char']) === false && empty($_POST['points_value']) === false) {
		...
		$yz85_account = mysql_select_single("SELECT `id`, `points` FROM `yz85_accounts` WHERE `account_id`='". $account['account_id'] ."';");
		data_dump($yz85_account, false, "Fetching existing points from yz85_accounts");

		data_dump(
			array(
				'Old:' => $yz85_account['points'],
				'New:' => $points, 
				'Total:' => ($yz85_account['points'] + $points)
				),
			false,
			"Points calculation:");
		$points += $yz85_account['points'];
		mysql_update("UPDATE `yz85_accounts` SET `points`='$points' WHERE `account_id`='". $account['account_id'] ."';");
	}
...
<p>
<?php
$basic = user_yz85_data('version', 'installed', 'cached');
if ($basic['version'] !== $version) {
	mysql_update("UPDATE `yz85` SET `version`='$version';");
	$basic = user_yz85_data('version', 'installed', 'cached');
}
echo "Running yz85 AAC Version: ". $basic['version'] .".<br>";
