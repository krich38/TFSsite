...
if (isset($_GET['success']) && empty($_GET['success'])) {
	...
	// Find a match
	$user = mysql_select_single("SELECT `id` FROM `yz85_accounts` WHERE `account_id`='$auid' AND `activekey`='$akey' AND `active`='0' LIMIT 1;");
	if ($user !== false) {
		$user = $user['id'];
		// Enable the account to login
		mysql_update("UPDATE `yz85_accounts` SET `active`='1' WHERE `id`='$user' LIMIT 1;");
